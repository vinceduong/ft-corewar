...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2018/12/10 18:40:33 by aetchego          #+#    #+#             */
/*   Updated: 2018/12/10 18:41:45 by aetchego         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	print_players_share(t_vm *vm)
{
	...
	i = 0;
	top = 72;
	left = vm->display.name_len + 8 + 7;
	while (i < vm->num_players)
	{
		move(top, left);
		create_jauge((float)vm->lives_total,
		(float)vm->players[i].nb_live, vm->players[i].nb_live);
		i++;
		top = top + 2;
	}
}
